...
#'   w<-pie.share.dist(3,5000)
#'   plot(w[,1],w[,2])
#'   
#' @keywords random weights
pie.share.dist<-function(N,M=1,randomnumbers=NA)
{
  ...
  if (is.na(randomnumbers)){
    ...
    if (len(randomnumbers != M*N-1)){
      ...
      print("pie.share.dist: random numbers provided need to be uniform distributed between 0 and 1")
      return (NA)
    }
